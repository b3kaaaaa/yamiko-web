...
// Removed date-fns to avoid build error with new module on uninstalled deps
...
export default function ShadowArmyPage() {
    ...
    // Locked if previous not met? Or just simple progress.
    // User requested: "Rewards should be reset if I am not at that level".
    // We will use strict conditions for styling.
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // We'll calculate locally for simplicity as RPC might not be applied correctly yet
    ...
    return (
        <>
            {/* Main Content (From User Code) */}

            {/* Main Content (From User Code) */}
            <main className="flex-1 min-w-0 mx-auto">
                {/* Hero Section */}
                <div className="relative w-full rounded-3xl overflow-hidden border border-primary/20 bg-surface-dark mb-10 shadow-2xl">
                    ...
                    <div className="relative z-10 p-12 flex flex-col md:flex-row items-center justify-between gap-10">
                        <div className="text-center md:text-left">
                            ...
                            <div className="mt-8 flex items-center gap-4 justify-center md:justify-start">
                                <div className="bg-surface-dark/80 backdrop-blur border border-white/5 rounded-xl px-5 py-3 text-center min-w-[140px]">
                        ...
                        <div className="relative flex-shrink-0 group">
                            ...
                            <div className="relative">
                                ...
                                <button onClick={copyCode} className="relative w-48 h-48 rounded-full bg-surface-dark border-2 border-primary/50 flex flex-col items-center justify-center gap-2 shadow-[0_0_40px_rgba(124,58,237,0.3),inset_0_0_20px_rgba(124,58,237,0.2)] hover:scale-105 transition-all duration-300 z-20 group active:scale-95 hover:shadow-[0_0_60px_rgba(124,58,237,0.5),inset_0_0_30px_rgba(124,58,237,0.4)]">
                ...
                <div className="mb-12">
                    ...
                    <div className="relative bg-surface-dark border border-white/5 rounded-2xl p-8 pb-12">
                        {/* Track */}
                        <div className="absolute left-10 right-10 top-12 h-1 bg-white/5 rounded-full"></div>
                        {/* Progress */}
                        <div className="absolute left-10 top-12 h-1 bg-gradient-to-r from-primary to-primaryHover rounded-full shadow-[0_0_15px_rgba(124,58,237,0.6)]" style={{ width: `${Math.min(100, Math.max(0, (stats.total / 100) * 100))}%` }}></div>
                        ...
                        <div className="relative w-full flex justify-between px-4">
                            ...
                            <div className="flex flex-col items-center gap-4 relative z-10">
                                <div className="w-8 h-8 rounded-full bg-primary border-2 border-white shadow-[0_0_10px_rgba(124,58,237,0.8)] z-10"></div>
                                <span className="text-xs font-bold text-primary absolute top-10 whitespace-nowrap">Начало</span>
                            </div>

                            {/* 10 Shadows */}
                            <div className="flex flex-col items-center gap-4 relative z-10">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-colors ${stats.total >= 10 ? 'bg-primary text-white' : 'bg-surface-highlight-dark border-2 border-gray-600 text-gray-500'}`}>
                            ...
                            {/* 50 Shadows */}
                            <div className="flex flex-col items-center gap-4 relative z-10">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-colors ${stats.total >= 50 ? 'bg-primary text-white' : 'bg-surface-highlight-dark border-2 border-gray-600 text-gray-500'}`}>
                            ...
                            {/* 100 Shadows */}
                            <div className="flex flex-col items-center gap-4 relative z-10">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-colors ${stats.total >= 100 ? 'bg-primary text-white' : 'bg-surface-highlight-dark border-2 border-gray-600 text-gray-500'}`}>
                            ...
                            {/* Crown */}
                            <div className="flex flex-col items-center gap-4 relative z-10">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors ${stats.total >= 500 ? 'bg-primary border-white' : 'bg-surface-highlight-dark border-gray-600'}`}>
                ...
                <div>
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-2xl font-black text-white flex items-center gap-3">
                            <span className="w-1.5 h-8 bg-primary rounded-full shadow-[0_0_15px_rgba(124,58,237,0.6)]"></span>
                            Моя Армия
                        </h3>
                        <div className="flex gap-3">
                            <div className="relative group">
                                <input className="bg-surface-dark border border-white/10 rounded-xl px-4 py-2.5 pl-10 text-sm text-white focus:outline-none focus:border-primary/50 w-64 placeholder-gray-600 transition-all group-hover:border-white/20" placeholder="Поиск тени..." type="text" />
                            ...
                            <button className="w-10 h-10 bg-surface-dark border border-white/10 hover:bg-white/5 hover:border-white/20 rounded-xl text-white transition-all flex items-center justify-center">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                        ...
                        {army.length > 0 ? (
                            army.map((soldier) => (
                                <div key={soldier.id} className="bg-surface-dark border border-white/5 rounded-2xl p-4 relative group hover:-translate-y-1 transition-all duration-300">
                                    ...
                                    <div className="flex flex-col items-center pt-2">
                                        <div className="relative w-20 h-20 mb-3 ml-auto mr-auto">
                                            ...
                                            {soldier.avatar_url ? (
                                                ...
                                                <div className="w-full h-full rounded-full bg-surface-highlight-dark border border-white/10 relative z-10 flex items-center justify-center">
                                            ...
                                            <div className="absolute -bottom-1.5 inset-x-0 flex justify-center z-20">
                                                <span className="bg-surface-dark border border-white/10 text-[10px] text-gray-300 px-2 py-px rounded-full font-bold">Ур. {soldier.level}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="w-full bg-surface-highlight-dark/50 rounded-lg p-2.5 flex justify-between items-center group-hover:bg-surface-highlight-dark transition-colors">
                        ...
                        <div onClick={copyCode} className="bg-surface-dark border border-white/5 border-dashed rounded-2xl p-4 relative group hover:bg-white/5 transition-all duration-300 flex flex-col items-center justify-center cursor-pointer min-h-[250px]">
                            <div className="w-16 h-16 rounded-full bg-surface-highlight-dark flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                                <span className="material-symbols-outlined text-3xl text-gray-600 group-hover:text-primary transition-colors">add</span>
                            </div>
            ...
            {/* Right Sidebar (Specific to Army Page) */}
            <aside className="hidden lg:block w-80 shrink-0 space-y-6 sticky top-24 self-start h-[calc(100vh-6rem)] overflow-y-auto no-scrollbar pb-10">
                ...
                <div className="bg-sidebar border border-white/5 rounded-2xl overflow-hidden shadow-lg flex flex-col">
                    ...
                    <div className="p-6 relative">
                        ...
                        <div className="relative pl-10 mb-8">
                            <div className={`absolute left-0 top-0 w-8 h-8 rounded-full flex items-center justify-center shadow-[0_0_10px_rgba(124,58,237,0.5)] z-10 ${isGeneral ? 'bg-primary text-white' : 'bg-surface-highlight-dark border-2 border-gray-600 text-gray-500'}`}>
                                {isGeneral ? <span className="material-symbols-outlined text-sm font-bold">check</span> : <span className="text-[10px]">100</span>}
                            </div>
                            <div className="mb-2">
                                <h4 className={`font-bold text-sm uppercase tracking-wide ${isGeneral ? 'text-primary' : 'text-gray-500'}`}>ГЕНЕРАЛ (100)</h4>
                            </div>
                            <div className={`bg-surface-highlight-dark/50 border border-white/5 rounded-xl p-3 flex items-center gap-3 ${isGeneral ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                        ...
                        <div className="relative pl-10 mb-8">
                            <div className={`absolute left-0 top-0 w-8 h-8 rounded-full flex items-center justify-center z-10 ${isMarshal ? 'bg-primary text-white' : (isGeneral ? 'bg-surface-highlight-dark border-2 border-primary text-primary shadow-[0_0_15px_rgba(124,58,237,0.4)]' : 'bg-surface-highlight-dark border-2 border-white/10 text-gray-500')}`}>
                            ...
                            <div className="mb-2 flex justify-between items-end">
                                ...
                                {!isMarshal && isGeneral && (
                                    <span className="text-xs text-primary font-bold">{Math.min(100, Math.floor(((stats.total - 100) / 50) * 100))}%</span>
                                )}
                            </div>
                            ...
                            {isGeneral && !isMarshal && (
                                <div className="w-full h-1.5 bg-white/10 rounded-full mb-3 overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-primary to-primaryHover shadow-[0_0_10px_rgba(124,58,237,0.5)]" style={{ width: `${Math.min(100, Math.floor(((stats.total - 100) / 50) * 100))}%` }}></div>
                                </div>
                            )}

                            <div className={`bg-surface-highlight-dark border border-white/10 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden group ${isGeneral ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                                {isGeneral && <div className="absolute inset-0 bg-primary/5 group-hover:bg-primary/10 transition-colors"></div>}
                        ...
                        <div className="relative pl-10">
                            <div className={`absolute left-0 top-0 w-8 h-8 rounded-full border-2 flex items-center justify-center z-10 ${isMonarch ? 'bg-primary border-primary text-white' : (isMarshal ? 'bg-surface-highlight-dark border-primary text-primary' : 'bg-surface-highlight-dark border-white/10 text-gray-500')}`}>
                            ...
                            <div className={`bg-surface-highlight-dark/30 border border-white/5 rounded-xl p-3 flex items-center gap-3 ${isMonarch || isMarshal ? 'opacity-100' : 'opacity-50'}`}>
                    ...
                    <div className="p-4 bg-surface-highlight-dark/30 border-t border-white/5 text-center">
                        <p className="text-[10px] text-gray-500">Награды выдаются автоматически при достижении цели</p>
                    </div>
                </div>

                {/* Summon Statistics Section */}
                <div className="bg-sidebar border border-white/5 rounded-2xl overflow-hidden shadow-lg p-5">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">СТАТИСТИКА ПРИЗЫВА</h3>
                    <div className="space-y-3">
                        <div className="bg-surface-highlight-dark/50 border border-white/5 rounded-xl p-3 flex items-center justify-between">
                        ...
                        <div className="bg-surface-highlight-dark/50 border border-white/5 rounded-xl p-3 flex items-center justify-between">
                            ...
                            <span className="text-primary font-bold">+{stats.week}</span>
                        </div>
                        <div className="bg-surface-highlight-dark border border-white/10 rounded-xl p-3 flex items-center justify-between">
