-- Migration: 23_fix_raid_hp_column.sql
-- Description: Fix undefined column error (current_boss_hp -> current_hp)

-- The previous migration (22) had a typo in the column name for guild_raids HP.
-- It used `current_boss_hp` but the table definition (migration 10) uses `current_hp`.
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'raid_id', gr.id,
        'boss_name', bd.name,
        'current_hp', gr.current_hp, -- FIXED: was current_boss_hp
        'max_hp', bd.hp,
