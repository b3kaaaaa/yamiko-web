-- Migration: 24_fix_boss_hp_column.sql
-- Description: Fix undefined column error (bd.hp -> gr.max_hp)

-- The function referenced `bd.hp` (from boss_definitions), but the column is `base_hp`.
-- However, we should actually be using `gr.max_hp` (from guild_raids) because it has the
-- scaled HP for that specific raid instance.
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'raid_id', gr.id,
        'boss_name', bd.name,
        'current_hp', gr.current_hp, -- Correct
        'max_hp', gr.max_hp, -- FIXED: using gr.max_hp instead of bd.hp (which doesn't exist, and base_hp is unscaled)
